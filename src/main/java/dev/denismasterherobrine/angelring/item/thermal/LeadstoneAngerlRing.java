...
public class LeadstoneAngerlRing extends Item {
    ...
    public ICapabilityProvider initCapabilities(final ItemStack stack, CompoundNBT unused) {
        if (ExternalMods.CURIOS.isLoaded()) {
            return LeadstoneAngelRingIntegration.initCapabilities(stack);
        }
    ...
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> tooltip, ITooltipFlag tooltipFlag) {
        ...
        if (!Screen.hasShiftDown())
            ...
            tooltip.add(new TranslationTextComponent("item.angelring.energetic_angel_ring.desc0").withStyle(TextFormatting.DARK_GREEN));
            tooltip.add(new TranslationTextComponent("item.angelring.leadstone_angel_ring.desc1", energy.getEnergyStored(), energy.getMaxEnergyStored()).withStyle(TextFormatting.GRAY));
        }
    }
}
